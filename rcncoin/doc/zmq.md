...
The RCNcoin Core daemon can be configured to act as a trusted "border
router", implementing the rcncoin wire protocol and relay, making
...
The ZeroMQ feature in RCNcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building rcncoind:
...
For instance:
    ...
    $ rcncoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/rcncoind.tx.raw \
...
These options can also be provided in rcncoin.conf.
...
From the perspective of rcncoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into rcncoind directly. Furthermore, no information is
...
using. RCNcoind appends an up-counting sequence number to each
