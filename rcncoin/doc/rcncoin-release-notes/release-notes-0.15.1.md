RCNcoin Core version *0.15.1* is now available from:

  <https://download.rcncoin.org/rcncoin-0.15.1/>
...
A number of changes to the way RCNcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 heaps per kB (0.00001 RCN). This is to prepare for dropping the minimum transaction fee to 0.1 heaps per kB (0.0001 RCN) in the next release.
