...
The interface runs on the same port as the JSON-RPC interface, by default port 9665 for mainnet, port 19332 for testnet,
...
* mempoolminfee : (numeric) minimum feerate (RCN per KB) for tx to be accepted
...
Risks
-------------
Running a web browser on the same node with a REST enabled rcncoind can be a risk. Accessing prepared XSS websites could read out tx/block data of your node by placing links like `<script src="http://127.0.0.1:9665/rest/tx/1234567890.json">` which might break the nodes privacy.
