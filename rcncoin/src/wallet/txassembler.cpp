    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional RCN outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes RCN and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::RCN_TO_RCN) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a RCN-to-RCN transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::RCN_ONLY;
        ...
        // If RCN-to-RCN fails, create a peg-out transaction
